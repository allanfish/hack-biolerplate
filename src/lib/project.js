...
/**
 * hack 项目相关
 * @namespace hack.project
 */

exports.DEFAULT_REMOTE_REPOS = 'http://hack.baidu.com/repos';

/**
 * 获取项目目录中，需要编译的文件集合，返回格式为对象，`key` 为文件基于项目目录的绝对路径， `value` 为文件对象。
 *
 * 当前 media 环境下面设置有 `project.files` 列表时，走 {@link hack.project.getSourceByPatterns}， 否则兼容 hack 2 中
 * 的用法，基于 `project.include` 和 `hack.exclude` 走 {@link hack.util.find} 查找。
 *
 * @name getSource
 * @function
 * @memberOf hack.project
 */
exports.getSource = function() {
  var patterns = hack.media().get('project.files');
  ...
  var root = exports.getProjectPath(),
    source = {},
    project_exclude = /^(\/output\b|\/hack-[^\/]+)$/,
    include = hack.config.get('project.include'),
    exclude = hack.config.get('project.exclude');
  if (hack.util.is(exclude, 'Array')) {
    project_exclude = [project_exclude].concat(exclude);
  } else if (exclude) {
    project_exclude = [project_exclude, exclude];
  }
  hack.util.find(root, include, project_exclude, root).forEach(function(file) {
    // 产生对应的 File对象
    file = hack.file(file);
    if (file.release) {
...
exports.getSourceByPatterns = function(patterns, opts) {
  patterns = patterns || hack.media().get('project.files');
  ...
  opts = _.assign({
    ...
    matchBase: true,
    ignore: hack.media().get('project.ignore', []).map(function(pattern) {
...
function getPath(root, args) {
  if (args && args.length > 0) {
    ...
    return hack.util(args);
  } else {
    return hack.util(root);
  }
}
...
function initDir(path, title) {
  if (hack.util.exists(path)) {
    if (!hack.util.isDir(path)) {
      hack.log.error('unable to set path[%s] as %s directory.', path, title);
    }
  } else {
    hack.util.mkdir(path);
  }
  path = hack.util.realpath(path);
  if (path) {
    return path;
  } else {
    hack.log.error('unable to create dir [%s] for %s directory.', path, title);
  }
}
...
exports.getProjectPath = function() {
  if (PROJECT_ROOT) {
    ...
    hack.log.error('undefined project root');
  }
};
...
exports.setProjectRoot = function(path) {
  if (hack.util.isDir(path)) {
    PROJECT_ROOT = hack.util.realpath(path);
  } else {
    hack.log.error('invalid project root path [%s]', path);
  }
};
...
/**
 * 获取临时文件夹根目录，若未手动设置，则遍历用户环境变量中['hack_TEMP_DIR', 'LOCALAPPDATA', 'APPDATA', 'HOME']这几项是否有设置，有则作为临时目录path，没有则以hack3/.hack-tmp作为临时文件夹
 * @function
 * @memberOf hack.project
 * @name getTempRoot
 */
exports.getTempRoot = function() {
  if (!TEMP_ROOT) {
    var list = ['hack_TEMP_DIR', 'LOCALAPPDATA', 'APPDATA', 'HOME'];
    var name = hack.cli && hack.cli.name ? hack.cli.name : 'hack';
    var tmp;
...
exports.lookup = function(path, file, silent) {
  var info = hack.uri(path, file ? file.dirname : exports.getProjectPath());

  /**
   * 当查找文件时派送, 可以扩展 hack 默认的查找文件功能。如：支持无后缀文件查找，支持 components 文件查找。
   * @event lookup:file
   * @property {Object} info 包含查找路径信息。
   * @property {File} file 文件对象。
   * @memberOf hack
   */
  hack.emit('lookup:file', info, file, silent);
  if (!info.id) {
