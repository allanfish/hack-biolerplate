...
var DEFAULT_SETTINGS = {
  ...
  project: {
    ...
    md5Connector: '_',
    files: ['**'],
    ignore: ['node_modules/**', 'output/**', '.git/**', 'hack-conf.js']
  },

  component: {
    skipRoadmapCheck: true,
    protocol: 'github',
    author: 'hack-components'
  },
  ...
  system : {
    repos : 'http://hack.baidu.com/repos'
  }
};
 ...
 * 配置类。
 * @class
 * @memberOf hack.config
 * @param {Object} [initialOpitions] 初始配置。
 * @example
 * var Config = hack.confilg.Config;
 ...
 */
var Config = Object.derive(/** @lends hack.config.Config.prototype */{
    ...
    if ('string' === typeof string && rVariable.test(string)) {
      ...
      return string.replace(rVariable, function(all, key) {
        ...
        if(typeof val === 'undefined'){
            hack.log.error('undefined property [%s].', key);
        } else {
            return val || '';
        }
      });
   ...
   *
   * ```
   * hack.config.set('a', 'sting');
   *
   * // 如果这样设置会报错。
   * hack.config.set('a.subpath', 2);
   * ```
   *
   * @param {String} path  配置项路径。
   * @param {Mixed} value 值
   * @example
   * hack.config.set('xxxx', 1);
   */
      ...
      if (path) {
        ...
        paths.forEach(function(key) {
          ...
          if (type === 'object') {
            ...
            hack.log.error('forbidden to set property[%s] of [%s] data', key, type);
          }
        });
        data[last] = value;
      }
  ...
  merge: function() {
    ...
    [].slice.call(arguments).forEach(function(arg) {
      if (typeof arg === 'object') {
        ...
        hack.log.warning('unable to merge data[' + arg + '].');
      }
    });
    return this;
  },
  ...
  media: function(group) {
    group = group || hack.project.currentMedia() || 'dev';
   ...
   * @example
   * hack.config.match('*.js', {
   *   release: 'static/$0'
   * });
   */
  match: function(pattern, properties, weight) {
    if (!pattern) {
      hack.log.error('Invalid selector, must be a `RegExp` or `glob`');
    ...
    if (!hack.util.is(properties, 'Object')) {
      hack.log.error('Invalid Properties, must be an `Object`');
    }
        ...
        if (typeof pattern === 'string') {
          ...
          if (~pattern.indexOf('(') && ~pattern.indexOf('{')) {
            pattern = minimatch.braceExpand(pattern).map(function(pattern) {
              return hack.util.glob(pattern);
            });
          } else {
            pattern = hack.util.glob(pattern);
          }
        }
  ...
  /**
   * 挂载 {@link https://github.com/search?q=hack3-hook&ref=opensearch hook} 插件
   * @param  {string} name     插件名称
   * @param  {Object} settings 插件配置项
   * @example
   * hack.config.hook('module');
   */
  hook: function(name, settings) {
    ...
    origin.push(hack.plugin(name, settings));
...
/**
 * config 配置对象，该对象为 {@link hack.config.Config} 实例，详情请查看 {@link hack.config.Config Config 类}说明。
