...
function getReleaseExt(ext) {
  if (ext) {
    var rExt = hack.media().get('project.ext' + ext);
    if (rExt) {
...
function normalizePath(path, reg, rExt) {
  return path.replace(reg, '').replace(/[:*?"<>|]/g, '_') + rExt;
}

/*
 * 给文件添加md5标识符
 * @param {String} path 文件路径
 * @param {Object} file hack File对象
 * @return {String} 带有md5 hash标识的路径
 */
function addHash(path, file) {
  var rExt = file.rExt,
      qRExt = hack.util.escapeReg(rExt),
      qExt = hack.util.escapeReg(file.ext),
      hash = file.getHash(),
      onnector = hack.media().get('project.md5Connector', '_'),
...
function getDomainsByPath(path) {
  var domain = hack.media().get('project.domain', {}),
      value = [];
  if (typeof domain === 'string') {
    value = domain.split(/\s*,\s*/);
  } else if (hack.util.is(domain, 'Array')) {
    value = domain;
  } else {
    hack.util.map(domain, function (pattern, domain) {
      if (pattern === 'image' && hack.util.isImageFile(path) || hack.util.glob(pattern, path)) {
        if (typeof domain === 'string') {
          value = domain.split(/\s*,\s*/);
        } else if (hack.util.is(domain, 'Array')) {
          value = domain;
        } else {
          hack.log.warning('invalid domain [' + domain + '] of [project.domain.' + pattern + ']');
        }
        return true;
      }
    });
...
/**
 * File，hack 编译过程中，文件会被此类进行封装，对于文件的操作，都是通过此类来完成。
 *
...
var File = Object.derive(function () {
  ...
  if (realpath.indexOf(root) === 0) {
    ...
    // cache apply matches.
    var matches = hack.media().getSortedMatches();
  ...
  if (subpath) {
    if (this.release === false && !this.url) {
      ...
      Object.defineProperty(this, 'url', {
        enumerable: true,
        get: function () {
          hack.log.error('unreleasable file [%s]', self.realpath);
        }
      });
    } else {
    ...
    // charset
    if (this._isText) {
      this.charset = (this.charset || hack.media().get('project.charset', 'utf8')).toLowerCase();
    }

    //file id
    var id = this.id || subpath.replace(/^\//, '');
    var ns = hack.media().get('namespace');
    if (ns) {
      id = ns + hack.media().get('namespaceConnector', ':') + id;
    }

    this.id = id;
    this.moduleId = this.moduleId || this.id.replace(/\.js$/i, '');
  }
}, /** @lends hack.file~File.prototype */{
   ...
   */
  exists: function () {
    return hack.util.exists(this.realpath);
  },
   ...
   */
  getMtime: function () {
    return hack.util.mtime(this.realpath) || new Date();
  },
   ...
   */
  isFile: function () {
    return hack.util.isFile(this.realpath);
  },
   ...
   */
  isDir: function () {
    return hack.util.isDir(this.realpath);
  },
  ...
  getContent: function () {
    if (typeof this._content === 'undefined') {
      this._content = hack.util.read(this.realpath, this.isText());
    }
  ...
  getHash: function () {
    if (typeof this._md5 === 'undefined') {
      Object.defineProperty(this, '_md5', {
        value: hack.util.md5(this.getContent()),
        writable: false
      });
    }
    return this._md5;
  ...
  getBase64: function (prefix) {
    ...
    if (prefix) {
      prefix = 'data:' + hack.util.getMimeType(this.rExt) + ';base64,';
    } else {
      prefix = '';
    }
    return prefix + hack.util.base64(this._content);
  },
  ...
  getHashRelease: function (release) {
    ...
    if (release) {
      ...
      hack.log.error('unreleasable file [%s]', this.realpath);
    }
  },
  ...
  addSameNameRequire: function (ext) {
    ...
    if (hack.util.isFile(this.realpathNoExt + ext) && caseSensiveFileExits(this.realpathNoExt + ext)) {
      path = './' + this.filename + ext;
    } else if (map = hack.media().get('project.ext')) {
      for (var key in map) {
        if (map.hasOwnProperty(key)) {
          var oExt = normalizeExt(key);
          var rExt = normalizeExt(map[key]);
          if (rExt === ext && hack.util.isFile(this.realpathNoExt + oExt) && caseSensiveFileExits(this.realpathNoExt + oExt)) {
      ...
      // 只能把当前目录下面的其他文件列出来了。
      //
      // 推荐像 hack2 中一样设置 project.ext 来提高性能。
      var pattern = this.subpathNoExt + '.*';
      var files = hack.project.getSourceByPatterns(pattern);
    ...
    if (path) {
      var info = hack.uri.getId(path, this.dirname);
...
/**
 * 用来创建 File 对象， 更多细节请查看 {@link hack.file~File File} 说明。
 *
 * ```js
 * var file = hack.file(root, 'static/xxx.js');
 * ```
 *
 * @see {@link hack.file~File File 类说明}
 ...
 * @return {@link hack.file~File File 对象}
 * @example
 * var file = hack.file.wrap('path of file');
 * var file2 = hack.file.wrap(file);
 * @memberOf hack.file
 * @name wrap
 */
module.exports.wrap = function (file) {
  if (typeof file === 'string') {
    ...
    hack.log.error('unable to convert [%s] to [File] object.', typeof file);
  }
};