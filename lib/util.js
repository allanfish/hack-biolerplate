 ...
 * @type {Array}
 */
var TEXT_FILE_EXTS = ['css', 'tpl', 'js', 'php', 'txt', 'json', 'xml', 'htm', 'text', 'xhtml', 'html', 'md', 'conf', 'po', 'config', 'tmpl', 'coffee', 'less', 'sass', 'jsp', 'scss', 'manifest', 'bak', 'asp', 'tmp', 'haml', 'jade', 'aspx', 'ashx', 'java', 'py', 'c', 'cpp', 'h', 'cshtml', 'asax', 'master', 'ascx', 'cs', 'ftl', 'vm', 'ejs', 'styl', 'jsx', 'handlebars', 'shtml', 'ts', 'tsx', 'yml', 'sh', 'es', 'es6', 'es7', 'map'],
...
/**
 * hack 中工具类操作集合。{@link https://lodash.com/ lodash} 中所有方法都挂载在此名字空间下面。
 * @param  {String} path
 * @return {String}
 * @example
 *   /a/b//c\d/ -> /a/b/c/d
...
_.touch = function (path, mtime) {
  ...
  if (mtime instanceof Date) {
    ...
    hack.log.error('invalid argument [mtime]');
  }
...
function getFileTypeReg(type) {
  var map = [],
      ext = hack.config.get('project.fileType.' + type);
  if (type === 'text') {
    map = TEXT_FILE_EXTS;
  } else if (type === 'image') {
    map = IMAGE_FILE_EXTS;
  } else {
    hack.log.error('invalid file type [%s]', type);
  }
  if (ext && ext.length) {
...
_.md5 = function (data, len) {
  ...
  len = len || hack.config.get('project.md5Length', 7);
...
_.read = function (path, convert) {
  ...
  if (_exists(path)) {
    ...
    hack.log.error('unable to read file[%s]: No such file or directory.', path);
  }
  return content;
};
  ...
  function normalize(pattern) {
    ...
    switch (type) {
      ...
      case '[object RegExp]':
        return pattern;
      default:
        hack.log.error('invalid regexp [%s].', pattern);
    }
  }
...
_.find = function (rPath, include, exclude, root) {
  ...
  if (path) {
    ...
    hack.log.error('unable to find [%s]: No such file or directory.', rPath);
  }
  return list.sort();
};
...
_.del = function (rPath, include, exclude) {
  ...
  if (rPath && _.exists(rPath)) {
    ...
    if (/^(?:\w:)?\/$/.test(path)) {
      hack.log.error('unable to delete directory [%s].', rPath);
    }
    ...
    //hack.log.error('unable to delete [' + rPath + ']: No such file or directory.');
  }
  return removedAll;
};
...
_.copy = function (rSource, target, include, exclude, uncover, move) {
  ...
  if (source) {
    ...
    hack.log.error('unable to copy [%s]: No such file or directory.', rSource);
  }
  return removedAll;
};
...
/**
 * 加载处理hack模块下的全部插件，如hack-plugin-*
 * @param  {String}   type     模块名
 * @param  {Function} callback 回调
 * @param  {Object}   def      模块获取的默认值 @see hack.config.get def
 * @memberOf hack.util
 * @name pipe
 * @function
 */
_.pipe = function (type, callback, def) {
  var processors = hack.media().get('modules.' + type, def);
  if (processors) {
    ...
    // 若type为多层级(ex: 'a.b')，获取hack.config中groups[defaultGroup]['modules']下一层配置项的名称
    type = type.split('.')[0];
    processors.forEach(function (obj, index) {
      ...
      if (typeof processor === 'string') {
        key = type + '.' + processor;
        processor = hack.require(type, processor);
      } else {
        key = type + '.' + index;
      }
...
_.download = function (url, callback, extract, opt) {
  ...
  var http = opt.protocol === 'https:' ? require('https') : require('http'),
      ...
      tmp = hack.project.getTempPath('downloads', name),
      data = opt.data;
  delete opt.data;
  _.write(tmp, '');
  var writer = fs.createWriteStream(tmp),
    ...
    if (callback) {
      ...
      hack.log.error('request error [%s]', msg);
    }
  },
        ...
        if (extract) {
          fs.createReadStream(tmp).pipe(getTar().Extract({
            ...
            if (callback) {
              ...
              hack.log.error('extract tar file [%s] fail, error [%s]', tmp, err);
            }
          }).on('end', function () {
...
_.upload = function (url, opt, data, content, subpath, callback) {
  if (typeof content === 'string') {
    ...
    hack.log.error('unable to upload content [%s]', typeof content);
  }
  opt = opt || {};
  data = data || {};
  var endl = '\r\n';
  ...
  _.download(url, function (err) {
    if (err) {
      if (opt.error) {
        ...
        hack.log.error('unable to download component [%s@%s] from [%s], error [%s].', name, version, url, err);
      }
    } else {
      if (opt.done) {
...
_.readJSON = function (path) {
  ...
  try {
    ...
    hack.log.error('parse json file[%s] fail, error [%s]', path, e.message);
  }
  return result;
};
 ...
 * @param  {String} pattern 符合hack匹配文法的正则串
...
_.glob = function (pattern, str, options) {
  ...
  if (~['::text', '::image', 'text', 'image'].indexOf(pattern)) {
    ...
    // 由于minimatch提供的glob支持中()语法不符合hack glob的需求，因此只针对()单独处理
...
_.nohup = function (cmd, options, callback) {
  ...
  if (IS_WIN) {
    var cmdEscape = cmd.replace(/"/g, '""'),
        file = hack.project.getTempPath('nohup-' + _.md5(cmd) + '.vbs'),
        script = '';
    script += 'Dim shell\n';
    ...
    return exec('cscript.exe /nologo "' + file + '"', options, function (error, stdout) {
      if (stdout != '0') {
        hack.log.error('exec command [%s] fail.', cmd);
      }
      fs.unlinkSync(file);
    ...
    return exec('nohup ' + cmd + ' > /dev/null 2>&1 &', options, function (error, stdout) {
      if (error !== null) {
        hack.log.error('exec command [%s] fail, stdout [%s].', cmd, stdout);
      }
      if (typeof callback === 'function') {
        callback();
      }
  ...
  matches.forEach(function (item, index) {
    ...
    if (match !== item.negate) {
      ...
      keys.forEach(function (key) {
        ...
        var value = typeof properties[key] === 'object' ? hack.util.cloneDeep(properties[key]) : properties[key];
        ...
        } else if (typeof value === 'function' && ~['release', 'url', 'relative', 'moduleId'].indexOf(key)) {
        ...
        if (value && value.__plugin && value.__pos) {
          ...
          hack.util.assign(obj[key], value);
        } else {
          obj[key] = value;
        }
      });
